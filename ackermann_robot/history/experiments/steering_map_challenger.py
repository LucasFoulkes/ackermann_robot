...
Fits a monotone piecewise-linear pulse->curvature map per direction from
...
time; the challenger predicts kappa = f_learned(commanded pulse) at the same
...
KNOT_SPACING_US = 30.0
MIN_BIN_N = 4                  # bins thinner than this lean on the prior

# Generic cautious prior: +-18 deg wheel angle over +-430 us of servo travel
# around an assumed 1470 us center. Deliberately NOT this robot's measured map.
PRIOR_CENTER_US = 1470.0
PRIOR_SLOPE_1PM_PER_US = math.tan(math.radians(18.0)) / WHEELBASE_M / 430.0

HELD_OUT_STAMPS = ('20260712_1359', '20260712_1518', '20260712_1543')


def eligible_samples(path):
    """Yield (direction, pulse_us_lag_aligned, measured_kappa) tuples."""
    try:
    ...
    numeric = ('monotonic_s', 'steering_us', 'accepted_curvature_1pm',
    ...
    pulses = [float(r['steering_us'] or 0) for r in rows]
    ...
    for r in rows:
        ...
        if abs(at(cmds, t - lag) - at(cmds, t)) > SETTLE_TOL_1PM:
            continue
        pulse = at(pulses, t - lag)
        if pulse < 900 or pulse > 2100:
            continue
        km = float(r['measured_w_radps'] or 0) / v
        if abs(km) > 2.5:
            continue
        out.append(('fwd' if v > 0 else 'rev', pulse, km))
    return out
...
def fit_direction(samples):
    ...
    if not samples:
        return None
    pulses = np.array([s[1] for s in samples])
    kappas = np.array([s[2] for s in samples])
    lo = math.floor(pulses.min() / KNOT_SPACING_US) * KNOT_SPACING_US
    hi = math.ceil(pulses.max() / KNOT_SPACING_US) * KNOT_SPACING_US
    knots = np.arange(lo, hi + KNOT_SPACING_US, KNOT_SPACING_US)
    means, weights = [], []
    for k in knots:
        m = np.abs(pulses - k) <= KNOT_SPACING_US
        n = int(m.sum())
        prior = (k - PRIOR_CENTER_US) * PRIOR_SLOPE_1PM_PER_US
        if n >= MIN_BIN_N:
    ...
    return {
        'knots_us': [round(float(k), 1) for k in knots],
...
def evaluate(path, models):
    ...
    for r in rows:
        ...
        if abs(km) > 2.5:
            continue
        kc = at(cmds, t - lag)
        kp = predict(models[d], at(pulses, t - lag))
...
def write_bootstrap_map():
    ...
    import yaml
    knots = [round(1050.0 + 30.0 * i, 1) for i in range(26)]
    memory = {
        'schema_version': 1,
    ...
    for direction in ('forward', 'reverse'):
        memory['directions'][direction] = {
            'knots_pulse_us': knots,
            'kappa_1pm': [round((k - PRIOR_CENTER_US)
                                * PRIOR_SLOPE_1PM_PER_US, 4) for k in knots],
...
def main():
    ...
    for p in train:
        ...
        if got:
            used_files += 1
        for d, pulse, km in got:
            samples[d].append((d, pulse, km))
    ...
    for d, m in models.items():
        if m:
            print(f"{d}: {len(m['knots_us'])} knots over "
                  f"{m['knots_us'][0]:.0f}-{m['knots_us'][-1]:.0f} us, "
                  f"{m['bins_with_data']} data-backed bins")
