...
- **SHIPPED 2026-07-04: planner honesty link.** navigation.launch.py derives
  Smac minimum_turning_radius + RPP min-radius at every launch from
  auto_calib's measured per-side steering gains (worst side, clamped
  [1.3, 4.0], ≥15 pairs/side + plausibility guards). Born from the servo
  transplant halving right curvature while the planner kept promising
  R=1.3 arcs the car drove wide into obstacles. Self-heals: fix the
  linkage, drive ~60 steady pairs, restart — the radius follows.
